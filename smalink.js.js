    ...
    try {
        ...
        /* ============================================================
           ...
           2) Both flows used a fixed timeout (800ms / 400ms*20) before
              ...
              this could read stale or incomplete data.
           3) (Round 2 fix) The first version of the fix above only
              declared "done" once shipment ROWS appeared. On an order
              with zero pick tickets, no rows ever appear, so it always
              burned the full ~10s ceiling before giving up -- which is
              exactly the "slow to create email" symptom. Fix: instead
              of waiting only for rows, watch the whole Pick-Tickets
              pane's content and consider it "loaded" once that content
              stops changing between two consecutive checks (~600ms
              apart) -- whether that settled content has rows in it or
              legitimately has none. Rows appearing is still treated as
              an instant done signal so the normal (has data) case is
              never slowed down.
        ============================================================ */
        ...
        /* Broader signature covering the whole Pick-Tickets tab pane (not just
           grid rows) so we can tell the difference between "still loading"
           and "genuinely empty, already loaded" -- both look like zero rows,
           but only one of them should make us keep waiting. */
        function pickTicketsPaneSignature() {
            var pane = document.getElementById("picktickets");
            if (pane) {
                return (pane.textContent || "").replace(/\s+/g, " ").trim().slice(0, 4000);
            }
        ...
        function waitForShipmentGrid(cb) {
            ...
            var attempts    = 0;
            var maxAttempts = 15; /* ~500ms head-start + 15*300ms ≈ 5s hard ceiling */
            var lastSig     = null;

            function poll() {
                attempts++;
                var rows = shipmentRows();
                var sig  = pickTicketsPaneSignature();

                /* Rows already present -- data has loaded, don't wait any longer. */
                var haveRows = rows.length > 0;

                /* Content unchanged since the previous check -- whatever state
                   the pane is in (rows, or a legitimate "no pick tickets" state)
                   has settled, so it's safe to read now. (attempts > 1 so we
                   always take at least one real look before declaring "stable".) */
                var stable = attempts > 1 && sig === lastSig;
                lastSig = sig;

                if (haveRows || stable || attempts >= maxAttempts) {
                    cb();
                } else {
                    setTimeout(poll, 300);
                }
        ...
        function orderViewer() {
            ...
            bCopyHtml.addEventListener("click", function () {
                ...
                /* account name / number row, always on top, from the
                   order-details page meta (same helper used elsewhere
                   on this page for Account Number / order info) */
                var m = meta();
                if (m.name || m.acct) {
                    var acctRow = document.createElement("tr");
                    var acctTd  = document.createElement("td");
                    acctTd.colSpan = 5;
                    acctTd.style.cssText =
                        "padding:8px 10px;background:#1a1a2e;color:#fff;" +
                        "font-weight:700;font-size:13px";
                    var acctParts = [];
                    if (m.name) acctParts.push("Account: " + m.name);
                    if (m.acct) acctParts.push("Account #: " + m.acct);
                    acctTd.textContent = acctParts.join("   |   ");
                    acctRow.appendChild(acctTd);
                    emailTbl.appendChild(acctRow);
                }
        ...
        /* ============================================================
           SHOPPING CART  —  Cart Order Email
           New feature: on https://www.smalink.com/ShoppingCart, build a
           customer-service email (or export) from the current cart,
           with checkboxes controlling which details are included.
        ============================================================ */

        /* Impersonated company name / account number, from the yellow
           "CURRENTLY IMPERSONATING ..." bar at the top of the page. */
        function cartCompany() {
            var bar = qs(".ImpersonationBar");
            if (!bar) return { name: "", acct: "" };
            var t = txt(bar);
            t = t.replace(/turn\s*off\s*$/i, "").trim();
            t = t.replace(/^currently\s+impersonating\s*/i, "").trim();
            var m = t.match(/^(.*)\((\d+)\)\s*$/);
            if (m) return { name: m[1].trim(), acct: m[2].trim() };
            return { name: t, acct: "" };
        }

        /* One row per .CartItem on the page */
        function cartItems() {
            var a = [];
            qsa(".CartItem").forEach(function (r) {
                var codeEl = r.querySelector(".CartItemCode strong");
                var descEl = r.querySelector(".CartItemDesc");
                var wtEl   = r.querySelector(".CartItemWeight");
                var qtyEl  = r.querySelector(".QtyInput");
                var upEl   = r.querySelector(".CartUnitPrice");
                var extEl  = r.querySelector(".CartItemExtendedPrice");
                if (!codeEl) return;

                a.push({
                    pn:  txt(codeEl),
                    sd:  descEl ? txt(descEl) : "",
                    qt:  qtyEl ? String(qtyEl.value || "").trim() : "",
                    wt:  wtEl  ? txt(wtEl).replace(/^total\s*weight:\s*/i, "").trim() : "",
                    up:  upEl  ? txt(upEl) : "",
                    ext: extEl ? txt(extEl).replace(/^ext\.?\s*/i, "").trim() : ""
                });
            });
            return a;
        }

        /* Cart-level totals from the totals card (CART WEIGHT / SUBTOTAL) */
        function cartTotals() {
            var out = { weight: "", subtotal: "" };
            qsa(".CartTotalWrapper li").forEach(function (li) {
                var t = txt(li);
                if (/cart weight/i.test(t))
                    out.weight = t.replace(/cart weight:\s*/i, "").trim();
                if (/subtotal/i.test(t))
                    out.subtotal = t.replace(/subtotal:\s*/i, "").trim();
            });
            return out;
        }

        /* Build the plain-text body used by both the email and the
           text/csv/Excel exports, from the checked field set. */
        /* Column set + row data, in your requested order:
           Qty, Part Number, Description, Weight, Price Each, Extended.
           Qty always shows; the rest follow the checkboxes. Shared by
           the email body and every export format so they always match. */
        function buildCartRows(f) {
            var it = cartItems();
            var header = ["Qty"];
            if (f.pn)   header.push("Part Number");
            if (f.desc) header.push("Description");
            if (f.wt)   header.push("Weight");
            if (f.up)   header.push("Price Each");
            if (f.ext)  header.push("Ext.");

            var rows = it.map(function (i) {
                var row = [i.qt];
                if (f.pn)   row.push(i.pn);
                if (f.desc) row.push(i.sd);
                if (f.wt)   row.push(i.wt);
                if (f.up)   row.push(i.up);
                if (f.ext)  row.push(i.ext);
                return row;
            });
            return { header: header, rows: rows };
        }

        /* Email-only variant: Qty and Part Number are merged into one
           "10 - 803-310605" cell instead of separate columns, per Paul's
           preference for a dash between them in the email body. The
           spreadsheet exports (cartExportRows/buildCartRows) keep Qty and
           Part Number as separate columns since that's more useful there. */
        function buildCartEmailRows(f) {
            var it = cartItems();
            var header = [f.pn ? "Qty - Part Number" : "Qty"];
            if (f.desc) header.push("Description");
            if (f.wt)   header.push("Weight");
            if (f.up)   header.push("Price Each");
            if (f.ext)  header.push("Ext.");
        ...
        /* Space-padded, fixed-width table -- reads like a simple Excel
           grid in a monospace mail body (Outlook/most mail apps use one
           for plain text). Not used for csv/Excel exports -- those stay
           tab/comma-delimited so they paste cleanly into a spreadsheet. */
        function padTable(header, rows) {
            var widths = header.map(function (h, idx) {
                var w = String(h).length;
                rows.forEach(function (r) {
                    w = Math.max(w, String(r[idx] || "").length);
                });
                return w;
            });
            function fmtRow(r) {
                return r.map(function (v, idx) {
                    var s = String(v || "");
                    return s + Array(widths[idx] - s.length + 3).join(" ");
                }).join("").replace(/\s+$/, "");
            }
            var lines = [fmtRow(header)];
            lines.push(widths.map(function (w) {
                return Array(w + 3).join("-");
            }).join(""));
            rows.forEach(function (r) { lines.push(fmtRow(r)); });
            return lines.join("\n");
        }

        function buildCartEmailBody(f) {
            var co  = cartCompany();
            var tot = cartTotals();
            var t   = buildCartEmailRows(f);
            var lines = [];

            if (f.company && co.name) {
                lines.push("Company: " + co.name + (co.acct ? " (" + co.acct + ")" : ""));
                lines.push("");
            }

            lines.push(padTable(t.header, t.rows));

            /* Cart Totals is opt-in (unchecked by default) -- only shown
               when Paul checks it, regardless of which columns are on. */
            if (f.totals) {
                var sumParts = [];
                if (tot.weight)   sumParts.push("Cart Weight: " + tot.weight);
                if (tot.subtotal) sumParts.push("Subtotal: "    + tot.subtotal);
                if (sumParts.length) {
                    lines.push("");
                    lines.push(sumParts.join("   |   "));
                }
            }
        ...
        /* Rows (array of arrays) for the txt/csv/Excel exports --
           same column order/content as the email, just undelimited. */
        function cartExportRows(f) {
            var co = cartCompany();
            var t  = buildCartRows(f);
            return { rows: [t.header].concat(t.rows), company: f.company ? co.name : "" };
        }

        function cartOrderEmailAction() {
            var it = cartItems();
            if (!it.length) { alert("No items found in cart."); return; }

            var p   = makePanel("Cart Order Email");
            var pan = p.panel;
            var body = el("div", S.scroll + ";padding:14px");

            var sep = el("div", S.sep);
            sep.textContent = "INCLUDE IN EMAIL / EXPORT";
            ...
            var cbCompany = checkRow("Company Name", true);
            var cbPn      = checkRow("Part Number", true);
            var cbDesc    = checkRow("Description", true);
            var cbWt      = checkRow("Weight", false);
            var cbUp      = checkRow("Price Each", false);
            var cbExt     = checkRow("Price Extended", false);
            var cbTotals  = checkRow("Cart Totals (Weight & Subtotal)", false);


            /* preview of item count so it's clear what will be included */
            var hint = el("div", "font-size:12px;color:#888;margin-top:4px;text-align:center");
            hint.textContent = it.length + " item" + (it.length === 1 ? "" : "s") + " in cart";
            ...
            /* Export button + dropdown (Copy for Excel / .txt / .csv / PDF) */
            var exportWrap = el("div", "position:relative;flex:1 1 auto");
            var bExport = el("button", S.btn);
            bExport.textContent = "Export \u25be";
            exportWrap.appendChild(bExport);
            ...
            var exportMenu = el("div",
                ...
                "background:#fff;border:1.5px solid #ddd;border-radius:10px;" +
                "box-shadow:0 -4px 16px rgba(0,0,0,.18);z-index:9999;overflow:hidden");
            exportWrap.appendChild(exportMenu);

            function exportItem(labelTxt, handler) {
                var item = el("div",
                    "padding:11px 14px;font-size:13px;font-weight:600;cursor:pointer;" +
                    "color:#1a1a2e;border-bottom:1px solid #f0f0f0");
                item.textContent = labelTxt;
                item.addEventListener("click", function () {
                    exportMenu.style.display = "none";
                    handler();
                });
            ...
            exportItem("Copy for Excel", function () {
                var data = cartExportRows(fields());
                copyText(data.rows.map(function (r) { return r.join("\t"); }).join("\n"),
                    function () {
                        bExport.textContent = "\u2713 Copied!";
                        setTimeout(function () { bExport.textContent = "Export \u25be"; }, 2000);
                    },
            ...
            /* Matches SMA's own Upload Cart format exactly (tab-delimited,
               item code first, quantity second, no header row) so this file
               can be re-imported later via the UPLOAD CART button. Always
               Part Number + Qty regardless of the checkboxes above, since
               that's what the importer expects. */
            exportItem("Download .txt (Re-import)", function () {
                var it = cartItems();
                var lines = it.map(function (i) { return i.pn + "\t" + i.qt; });
                var blob = new Blob([lines.join("\n")], { type: "text/plain" });
                var url  = URL.createObjectURL(blob);
                var a    = el("a");
                a.href = url;
                a.download = "cart_reimport_" + new Date().toISOString().slice(0, 10) + ".txt";
                document.body.appendChild(a); a.click(); document.body.removeChild(a);
                setTimeout(function () { URL.revokeObjectURL(url); }, 2000);
            });

            exportItem("Download .csv", function () {
                var data = cartExportRows(fields());
                function g(t) { return '"' + String(t || "").replace(/"/g, '""') + '"'; }
                var csv = data.rows.map(function (r) { return r.map(g).join(","); }).join("\n");
                var blob  = new Blob([csv], { type: "text/csv" });
                var url   = URL.createObjectURL(blob);
                var isIOS = /ipad|iphone|ipod/i.test(navigator.userAgent);
                if (isIOS) {
            ...
            exportItem("Save as PDF", function () {
                var origLabel = "Export \u25be";
                function buildPdf() {
                    var f    = fields();
                    var data = cartExportRows(f);
                    var tot  = cartTotals();
                    var doc  = new window.jspdf.jsPDF({ unit: "pt", format: "letter" });
                    var marginX = 40, y = 50;
                    ...
                    doc.setFont("helvetica", "bold");
                    doc.setFontSize(16);
                    doc.text("Cart Order Summary", marginX, y);
                    y += 24;

                    doc.setFontSize(10);
                    doc.setFont("helvetica", "normal");
                    if (data.company) { doc.text("Company: " + data.company, marginX, y); y += 14; }
                    ...
                    doc.autoTable({
                        ...
                        margin: { left: marginX, right: marginX },
                        head: [data.rows[0]],
                        body: data.rows.slice(1),
                        styles: { font: "helvetica", fontSize: 9, cellPadding: 5, valign: "top" },
                        headStyles: { fillColor: [26, 26, 46], textColor: 255, fontStyle: "bold" }
                    });

                    var todayStr = new Date().toISOString().slice(0, 10);
                    var safeName = (data.company || "cart")
            ...
            bExport.addEventListener("click", function (ev) {
                ev.stopPropagation();
                var isOpen = exportMenu.style.display === "block";
                exportMenu.style.display = isOpen ? "none" : "block";
            });
            document.addEventListener("click", function (ev) {
                if (!exportWrap.contains(ev.target)) exportMenu.style.display = "none";
            });
            ...
            bOk.addEventListener("click", function () {
                var f  = fields();
                var co = cartCompany();
                var bodyTxt = buildCartEmailBody(f);
                p.close();
                compose(
                    "smaorders@smalink.com",
                    "Order Request" + (f.company && co.name ? " — " + co.name : ""),
                    bodyTxt
                );
            });
        }

        if (PAGE.isShoppingCart) {
            pageActions.push(
                { label: "🛒  Cart Order Email", fn: cartOrderEmailAction }
            );
        }

        /* ============================================================
           SMALINK.COM ITEM PAGE  —  ETA Request
           New feature: on a smalink.com/p/... item page, send an ETA
           request built the same way as etaPicker(), but using the
           impersonated company (name + account #) from the
           .ImpersonationBar instead of order meta, since these pages
           aren't tied to a specific order.
        ============================================================ */

        /* Item Code from the item detail page's spec list:
           <li class="list-group-item"><b>Item Code:</b> <span ...>023-44119</span></li> */
        function smalinkItemCode() {
            var lis = qsa(".list-group-item");
            for (var idx = 0; idx < lis.length; idx++) {
                var b = lis[idx].querySelector("b");
                if (b && /item code/i.test(txt(b))) {
                    var sp = lis[idx].querySelector("span");
                    if (sp) return txt(sp);
                }
            }
            return "";
        ...
        function smalinkEtaPicker(pn) {
            ...
            function radioRow(labelTxt, val, checked) {
                var wrap = el("div",
                    ...
                    "cursor:pointer;background:#fafafa");
                var rb = el("input", "width:20px;height:20px;cursor:pointer");
                rb.type = "radio"; rb.name = "smalinkEtaTo"; rb.value = val;
                if (checked) rb.checked = true;
                var lbl = el("span", "font-size:14px;color:#333");
                lbl.textContent = labelTxt;
                wrap.appendChild(rb); wrap.appendChild(lbl);
            ...
            bCa.addEventListener("click", p.close);
            bOk.addEventListener("click", function () {
                var to = ra.checked ? ra.value : rc.value;
                var isTech = rc.checked;
                p.close();
                if (isTech) {
                    compose(
        ...
        if (PAGE.isSmalinkItem) {
            pageActions.push({
                label: "⏱  ETA / Tech Request",
                fn: function () {
                    var pn = smalinkItemCode();
                    if (!pn) pn = prompt("Part Number:", "");
                    if (!pn) return;
                    smalinkEtaPicker(pn);
                }
        ...
        quickLinks = quickLinks.concat([
            ...
            {
                label: "🧾  Expense Report Creator",
                fn: function () {
                    window.open("https://wildref.us/tools/expense-report.html", "_blank");
                }
            }
        ]);
